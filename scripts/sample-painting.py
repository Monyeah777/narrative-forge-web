...
Pipeline (handbook B1-2):
  1. Load painting/01-dallas.jpg (already EXIF-baked, HSV S×0.88)
  ...
  5. 3-round density-scaled neighbor repulsion + eligible-cell hole fill
...
WEIGHT_GAIN = 0.4
WEIGHT_EXP = 1.2
NOTE = "normalized 0..1 · OKLab-weighted · dark-culled · hole-relaxed · palette-quantized"
VOID = (5, 5, 5)  # #050505
RELAX_DEFAULT = 3
HOLE_CELL_PX = 16
HOLE_EMPTY_FRAC_MAX = 0.02
...
def hole_metrics(
    ...
    w: int,
    h: int,
    cell: int = HOLE_CELL_PX,
) -> dict:
    """Occupancy of L≥dark_L cells. Empty eligible cells read as black holes on #050505."""
    gh, gw = max(1, h // cell), max(1, w // cell)
    counts = np.zeros((gh, gw), dtype=np.int32)
    px = np.clip(np.floor(xs * w).astype(np.int32), 0, w - 1)
    py = np.clip(np.floor(ys * h).astype(np.int32), 0, h - 1)
    cx = np.clip(px // cell, 0, gw - 1)
    cy = np.clip(py // cell, 0, gh - 1)
    np.add.at(counts, (cy, cx), 1)

    elig = np.zeros((gh, gw), dtype=bool)
    for gy in range(gh):
        y0, y1 = gy * cell, min(h, (gy + 1) * cell)
        for gx in range(gw):
            x0, x1 = gx * cell, min(w, (gx + 1) * cell)
            elig[gy, gx] = bool(eligible[y0:y1, x0:x1].mean() >= 0.5)
    empty = elig & (counts == 0)
    n_elig = int(elig.sum())
    ...
    return {
        ...
        "empty_eligible": n_empty,
        "empty_eligible_frac": round(n_empty / max(n_elig, 1), 6),
        "max_count": int(counts.max()) if counts.size else 0,
        "mean_count_eligible": round(float(counts[elig].mean()) if n_elig else 0.0, 4),
        "p90_count_eligible": round(float(np.percentile(counts[elig], 90)) if n_elig else 0.0, 4),
    }
...
def relax_points(
    ...
    """Density-scaled neighbor push (handbook: 3 rounds, 限幅).

    Secord 2002 §1.1: unweighted Lloyd equalizes spacing and blurs the image.
    Bridson 2007: voids come from clustering. Local min distance ∝ 1/√ρ so
    bright cells stay denser (handbook w = 0.6+0.4·L^1.2) while clumps open.
    """
    if rounds <= 0:
        return xy
    pts = xy.copy()
    n = pts.shape[0]
    s = 1.0 / math.sqrt(n)
    w_norm = np.clip(weights / max(float(weights.mean()), 1e-9), 0.25, 4.0)
    min_d = 0.72 * s / np.sqrt(w_norm)
    for _ in range(rounds):
        cell = float(np.median(min_d))
        cell = max(cell, 1e-4)
            ...
            for ox in (-1, 0, 1):
                for oy in (-1, 0, 1):
                    for j in buckets.get((cx + ox, cy + oy), ()):
                        ...
                        thresh = 0.5 * (float(min_d[i]) + float(min_d[j]))
                        if dist < 1e-12 or dist >= thresh:
                            continue
                        push = (thresh - dist) * 0.5
                        cap = 0.5 * thresh
                        if push > cap:
                            push = cap
                        nrm = dxy / dist
...
def fill_holes(
    ...
    """Move surplus points from overcrowded eligible cells into empty ones.

    Does not add points (count stays 12k). Never places into L<dark_L majority cells.
    """
    pts = xy.copy()
    n = pts.shape[0]
    gh, gw = max(1, h // cell), max(1, w // cell)

    def occupancy(points: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        counts = np.zeros((gh, gw), dtype=np.int32)
        px = np.clip(np.floor(points[:, 0] * w).astype(np.int32), 0, w - 1)
        py = np.clip(np.floor(points[:, 1] * h).astype(np.int32), 0, h - 1)
        cx = np.clip(px // cell, 0, gw - 1)
        cy = np.clip(py // cell, 0, gh - 1)
        np.add.at(counts, (cy, cx), 1)
        return counts, cx, cy
    ...
    moved = 0
    for _ in range(4):
        counts, cx, cy = occupancy(pts)
        empty = np.argwhere(elig & (counts == 0))
        if empty.size == 0:
            break
        crowded = np.argwhere(elig & (counts >= 2))
        if crowded.size == 0:
            break
        crowded = crowded[np.argsort(-counts[crowded[:, 0], crowded[:, 1]])]
        for k, (ey, ex) in enumerate(empty):
            if k >= crowded.shape[0]:
                break
            oy, ox = crowded[k]
            members = np.flatnonzero((cy == oy) & (cx == ox))
            if members.size <= 1:
                continue
            pick = int(members[int(rng.integers(0, members.size))])
            jitter = rng.random(2) * 0.4 + 0.3
            pts[pick, 0] = np.clip((ex + jitter[0]) * cell / w, 0.0, 1.0)
            pts[pick, 1] = np.clip((ey + jitter[1]) * cell / h, 0.0, 1.0)
            moved += 1
    return pts, moved
...
def write_previews(
    ...
    hb = stats.get("holes_before") or {}
    ha = stats.get("holes_after") or {}
    lines = [
        f"cull L<{stats['dark_L']}: {stats['dark_cull_pct']:.3f}%   "
        f"k={stats['k']}   seed=0x{stats['seed']:X}   relax={stats['relax_rounds']}",
        f"ΔE_ok×100 mean={de['mean']:.3f}  max={de['max']:.3f}   "
        f"hue |mean|={hue['total_abs_mean_signed_deg']:.3f}°",
        f"holes empty-elig {hb.get('empty_eligible')}→{ha.get('empty_eligible')}  "
        f"frac {hb.get('empty_eligible_frac')}→{ha.get('empty_eligible_frac')}  "
        f"moved={stats.get('holes_moved')}",
    ]
    y0 = bar + col_h + 10
...
def sample_image(
    ...
    *,
    ...
    sample_w = WEIGHT_BASE + WEIGHT_GAIN * np.power(np.clip(sample_lab[:, 0], 0.0, None), WEIGHT_EXP)

    eligible_mask = (~dark_mask).reshape(h, w)
    holes_before = hole_metrics(xs, ys, eligible_mask, w, h)

    xy = np.stack([xs, ys], axis=1)
    xy = relax_points(xy, sample_w, relax_rounds, rng)
    xy, holes_moved = fill_holes(xy, eligible_mask, w, h, rng) if relax_rounds > 0 else (xy, 0)
    xs, ys = xy[:, 0], xy[:, 1]
    holes_after = hole_metrics(xs, ys, eligible_mask, w, h)
    ...
    stats = {
        ...
        "checks": {
            ...
            "empty_eligible_frac_le_2pct": holes_after["empty_eligible_frac"] <= HOLE_EMPTY_FRAC_MAX,
            "holes_not_worse": holes_after["empty_eligible"] <= holes_before["empty_eligible"],
        },
    }
...
def run(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--relax",
        type=int,
        default=RELAX_DEFAULT,
        help="density-scaled neighbor-repulsion rounds (handbook: 3 when voids/clumps show)",
    )
    ...
    if args.self_test:
        self_test()
        print("exif/oklab/cull fixture: ok")
        return 0
    ...
    print(json.dumps({k: stats[k] for k in ("id", "w", "h", "count", "k", "dark_cull_pct", "relax_rounds", "holes_before", "holes_after", "holes_moved", "deltaE_ok_100", "hue_shift_deg", "kcenter_max_deltaE_ok_100", "checks", "bytes", "gzip_bytes")}, indent=2))

    checks = stats["checks"]
    # Mean ΔE and hue total are the hard quality bars. Max ΔE and 3–6% cull
    # are reported; this Dallas bake has almost no L<0.19 mass, and a 256-color
    ...
    if args.relax > 0 and not checks["empty_eligible_frac_le_2pct"]:
        raise SystemExit(
            f"empty eligible cell frac {stats['holes_after']['empty_eligible_frac']} > {HOLE_EMPTY_FRAC_MAX}"
        )
    return 0
