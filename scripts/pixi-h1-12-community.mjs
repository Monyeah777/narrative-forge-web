#!/usr/bin/env node
/** H1-⑫ live COMMUNITY 100k Met v3 bin; INTRO Dallas + HALL Scotland 100k kept. */
...
const COMM_PAGE = path.join(PROTO, "pixi-community/index.html");
...
const PORT = Number(process.env.NF_BENCH_PORT || 8789);
...
function grepCommPage() {
  const src = fs.readFileSync(COMM_PAGE, "utf8");
  const hits = {
  ...
  const required = {
    ...
    hook: /__nfPixiCommunity/.test(src),
  };
  return {
    hits,
    required,
...
function grepLive() {
  ...
  const hits = {
    ...
    egg_json: /points_nf\.json/.test(src),
  };
  return {
    ...hits,
...
async function main() {
  ...
  const met = binRecord(MET, 100000, FROZEN_MET);
  const grepPage = grepCommPage();
  ...
  if (!scotland.pass) throw new Error("Scotland 100k drifted: " + JSON.stringify(scotland));
  if (!met.pass) throw new Error("Met 100k failed: " + JSON.stringify(met));
  if (!grepPage.pass) throw new Error("community page grep failed: " + JSON.stringify(grepPage));
  ...
  try {
    ...
    await iso.goto(`http://127.0.0.1:${PORT}/pixi-community/index.html`, {
      waitUntil: "domcontentloaded",
      timeout: 60_000,
    });
    const isoUntil = Date.now() + 30000;
    let communityIso = null;
    while (Date.now() < isoUntil) {
      communityIso = await iso.evaluate(() => window.__nfPixiCommunity || null);
      if (communityIso && communityIso.ok && communityIso.count === 100000) break;
      await new Promise((r) => setTimeout(r, 100));
    }
    await new Promise((r) => setTimeout(r, 1500));
    communityIso = (await iso.evaluate(() => window.__nfPixiCommunity || null)) || communityIso;
    const shotIso = uniqueArt("pixi_h1_12_community_iso_100k.png");
    if (fs.existsSync(ART)) await iso.screenshot({ path: shotIso, fullPage: false });
    await iso.close().catch(() => {});

    const live = await openLive(browser, "autoplay=0&dpr=1");
    ...
    const shotIntro = uniqueArt("pixi_h1_12_live_intro_100k.png");
    if (fs.existsSync(ART)) await live.page.screenshot({ path: shotIntro, fullPage: false });
    ...
    const shotHall = uniqueArt("pixi_h1_12_live_hall_100k.png");
    if (fs.existsSync(ART)) await live.page.screenshot({ path: shotHall, fullPage: false });

    await live.page.evaluate(() => {
      if (typeof window.assemble === "function") window.assemble(window.TARGETS.COMMUNITY);
    });
    const communityLive = await waitLive(
      live.page,
      (s) =>
        s.target &&
    ...
    const shotCommunity = uniqueArt("pixi_h1_12_live_community_100k.png");
    if (fs.existsSync(ART)) await live.page.screenshot({ path: shotCommunity, fullPage: false });
    ...
    const shotRb = uniqueArt("pixi_h1_12_live_bin0_community_12k.png");
    if (fs.existsSync(ART)) await rb.page.screenshot({ path: shotRb, fullPage: false });
    await rb.page.close().catch(() => {});

    const report = {
      task: "H1-⑫ live COMMUNITY 100k Met v3 bin",
      handbook: "docs/NF_PixiJS迁移总手册_整合版_v1_2026-09-11.md",
      timestamp: new Date().toISOString(),
      vendorSha,
      golden,
      bins: { dallas: dallas, scotland: scotland, met: met },
      grepPage,
      liveGrep,
      isolated: { community: communityIso, pageErrors: isoErrors, screenshot: shotIso },
      live: {
        intro: introFormed,
        hall: hallLive,
        community: communityLive,
        rollbackCommunity: rbComm,
        introErrors: live.pageErrors,
      ...
      screenshots: {
        ...
        live_community_100k: shotCommunity,
        live_bin0_community_12k: shotRb,
      },
      note: {
        intro: "Dallas 100k bin unchanged",
        hall: "Scotland 100k bin unchanged",
        community: "Met 100k bin is live default; ?bin=0 keeps 12k JSON",
        egg: "still points_nf.json",
      ...
      acceptance: {
        ...
        scotland_frozen: scotland.pass,
        met_bin: met.pass,
        grep_page: grepPage.pass,
        live_grep: liveGrep.pass,
        iso_ok: !!(communityIso && communityIso.ok && communityIso.count === 100000 && communityIso.id === "four-trees-met"),
        iso_webgl: !!(communityIso && communityIso.rendererName === "webgl"),
        iso_version: !!(communityIso && communityIso.version === "8.20.1"),
        ...
        community_met: !!(communityLive && communityLive.points && communityLive.points.id === "four-trees-met" && communityLive.points.count === 100000 && communityLive.points.source === "bin"),
        community_formed: !!(communityLive && communityLive.mode === "formed"),
        community_target: !!(communityLive && communityLive.target && communityLive.target.key === "COMMUNITY"),
        rollback_community_12k: !!(rbComm && rbComm.points && rbComm.points.count === 12000 && rbComm.points.source === "fetch" && rbComm.points.id === "four-trees-met"),
        no_pageerror_live: live.pageErrors.length === 0,
    ...
    fs.writeFileSync(path.join(ROOT, "painting/pixi-h1-12-community.json"), JSON.stringify(report, null, 2) + "\n");
