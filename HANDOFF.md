...
- DPR≤3 + H2 文件头：本条分支 `cursor/pixi-h1-dpr-header-52ec`（叠在 `cursor/pixi-h1-ctx-dead-52ec` 上）。`quantizeDpr` 原生整数 ≤3；`?dpr=` 仍是实验闸。所有 tracked `points.bin` 前缀 9B 头，payload 字节未改（旧 sha 记在 `payloadSha256`）。C2 golden 未动。现网点数仍 100k。证据 `painting/pixi-h1-14-approve.json`。
...
3. 只做用户这一次点名的事。现网默认 Pixi，介绍/大厅/社区默认 100k bin。`points.bin` 现有 9B `NFPT` 头。不要改弹簧。不要删 `physics()` / `applySpring`。不要把 150k/250k 接进现网，除非作者给出双端 60fps 数字。
