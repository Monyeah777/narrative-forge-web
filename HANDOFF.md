...
- **第二十轮 立体动态粒子演示（作者「额外搞一个有带立体动态粒子炫酷效果的」）**：㉛ 新增 `prototype/particles3d.html`（独立页、零依赖、不碰现网）。技法：点精灵 + 尺寸衰减 `gl_PointSize ∝ 1/w`、**加法混合 + 片元软圆点（顺序无关、免排序）**、GPU 端解析流场（涡旋 + 3D 正弦势场 curl 抖动，curl noise 同 Bridson 2007；本仓库 2D 版已在用）+ 呼吸脉冲、深度雾 + 径向冷暖着色；每帧只改 uniform、单次 draw call（CPU/帧≈0）。含**上下文丢失恢复**（`webglcontextrestored` 重建 program/buffer/uniform——本环境 SwiftShader 启动必丢一次，恢复后正常渲染）。验收 7/7：WebGL2 建立、120,000 粒、**glErr 0 / 0 JS 错误**、软件渲染下 60fps、13.77% 亮点、拖拽后平均亮度差 4.53。观感标定：初版点径 26/alpha 0.16–1.0 被加法刷白（整帧亮度 119、99% 像素超阈）→ 改点径 14/alpha 0.03–0.15 后整帧 19.5、**环状轮廓**（半径分箱亮点 42→**55**→36%）。参数 `?n=`、`?simple=1`；拖拽旋转 / 滚轮缩放 / 空格暂停。证据 `painting/p3d-demo.json`、截图 `nf-baseline-artifacts/particles3d-{1,2}.png`、`p3d-profile.png`。
- **第二十一轮 平面画作 → 3D（浮雕）＋切换键（作者澄清「是把原来的平面粒子转换成 3d」「弄一个按钮切换 3d」）**：㉜ 引擎抽为 `prototype/nf-particles3d.js`（两种模式）：**浮雕模式**把**同一批画作粒子**立体化——x/y = 画作归一坐标（按纵横比等比、不拉伸）、**z = 该粒子调色板颜色的亮度**（明凸暗凹），配色沿用画作自身 256 色；动态 = z 呼吸 + ±12° 轻微摆动，可拖拽环绕/滚轮缩放。㉝ 放映条新增「**3D 粒子 / 回到画作**」键（aria-pressed）：开 → 惰性载入引擎、把当前画作喂进去、停 2D 循环；关 → dispose 释放上下文、唤醒 2D；500ms 轮询 `__nfPoints.id` 使浮雕跟着放映换幅。**★ 实测修掉一个真 bug**：叠加层最初 `position:fixed + z-index:35` 会盖住放映条 → 「回到画作」点不到（切回后 `__nf3d.on` 仍 true）；改为插到 `.stage` 内 `.paper` **之前且不设 z-index**（同层按 DOM 顺序叠放）→ 盖住粒子画布、仍在 `.paper(z-index:1)` 之下，UI 始终可点。验收 `check-3d-toggle.js` **9/9**（含「3D = 同一批画作粒子：relief、50,601 粒、paintingId === pointsId」）；抽象演示页 7/7 仍通过。参数 `?p3d=1/0`、`?p3ddepth=`、`?p3dsize=`、`?p3dn=`。证据 `painting/p3d-demo.json`（`relief_mode_2026-09-13`）、截图 `3d-on.png`/`3d-off.png`。
- **第二十二轮 「没变出来 3D」修根因（作者：「没变出来 3d 呀」）**：㉞ **根因是数据格式不匹配**——`window.__nfPoints.points` 是 `[[x,y,i], …]` **三元组数组**，而 `setPainting` 只按「扁平数字数组」解析 → `pts[k].x` 全 `undefined` → 坐标与浮雕高度 z 全变 **NaN** → 画面自然是平的（实测深度 A/B：0.06 平均亮度差、0.03% 像素 = 完全无变化）。㉟ 修法：`setPainting` 同时认 `[[x,y,i],…]` / 扁平数组 / `{x,y,i}` 对象数组三种载体；顺带修**第二因素**——默认相机原为正面（yaw=0/pitch=0.12）看不出起伏，改为 **3/4 侧视（yaw 0.62 / pitch 0.34）+ 慢自转 0.05rad/s**；浮雕幅度统一由 `depth` 缩放（默认 0.42 → **0.95**）并加「凸起更亮 + 点更大」的深度明暗；`setDepth` 现在会**重传顶点缓冲**（此前只改 uniform，改深度无效果）。复验：`check-3d-relief.js` **5/5**（深度 A/B 平均亮度差 **3.13**、变化像素 **6.99%**；画作亮点 5.55% → **13.09%**），`check-3d-toggle.js` **9/9** 仍全绿。证据 `painting/p3d-demo.json`（`bugfix_2026-09-13_flat_relief`）、截图 `3d-relief-deep.png` vs `3d-relief-flat.png`。
- **第二十三轮 环绕（作者「我希望可以变成环绕的」）**：㊱ 浮雕模式自转 0.05 rad/s（一圈 126s，看不出来）→ **默认 0.28 rad/s ≈ 16°/s（一圈 ≈ 22s）**；拖拽接管（暂停环绕）、松手继续；新增 `?p3dorbit=<rad/s>` 与 `nf3d({orbit})`（`0` = 静止仅手动转，负值反向）。验收 `check-3d-orbit.js` **5/5**：orbit=0.280 rad/s（一圈 22.4s）｜4 秒转过 **71.4°**｜画面随时间明显变化（平均亮度差 3.78、变化像素 5.87%）｜0 页面错误。证据同上（`orbit_2026-09-13`）、截图 `3d-orbit-a.png`／`3d-orbit-b.png`。
- **第二十四轮 取消环绕 + 浮雕极性交叠（作者「还是不环绕了吧，但你这个明凸暗凹可以交叠运动 明凹暗凸」）**：㊲ 浮雕**默认不自动环绕**（`autoYaw=0`，可拖拽自由转；`?p3dorbit=` 可再开）；㊳ **极性交叠**：`p.z *= uFlip`、`uFlip = cos(2πt/T)`，默认 **T=18s**（9s 明凸暗凹 → 过零成平面 → 9s 明凹暗凸），深度明暗同步跟随极性（凸起始终更亮）；呼吸 + ±12° 轻摆保留。参数 `?p3dflip=<0–120s>`（0 固定）、`nf3d({flip})`。验收 `check-3d-flip.js` **6/6**：取到两极值（flipNow 0.992/−0.991）｜**两极值帧 corr(+1,−1)=0.22**（仅粒子像素，1=完全相同）｜正极帧与平面帧同向（0.388）｜默认 orbit=0｜周期可调｜0 错误。回归：`check-painting3d.js` **6/6**、`check-3d-toggle.js` **9/9**。证据同上（`flip_2026-09-13`）、截图 `3d-flip-positive.png`／`3d-flip-negative.png`。
- **第二十五轮 3D 开关 / 环绕 / 亮度 / 节奏四修（作者「点击 3d 粒子按钮时无法切换，然后还是在环绕，而且亮度太低，需要交替的粒子节奏动感」）**：
  - ㊴ **切换硬化**：`set3D` 改**乐观置位** —— 点击立即变「回到画作」并显示「3D 载入中…」，载入失败则**回滚**状态并显示「3D 不可用」（不再静默无反应）；载入中重复点击忽略；500ms 发布定时器只建一次。
  - ㊵ **环绕**：根因是 `setMode("relief")` 无条件把 `autoYaw` 置 0，**连 `?p3dorbit=` 的显式值一起覆盖**（实测传 0.3 仍是 orbit=0）。改为记住显式设定（`orbitSet`），只有没显式设定时才按模式取默认（浮雕 0 / 抽象 0.045）；`setOrbit()` 同步更新该设定，显式值此后不再被模式切换吃掉。
  - ㊶ **亮度标定**：点径 4.2 → **5.0**、亮度倍率 1.7 → **2.5**；整帧平均亮度 16.5 → **24.2（+47%）**、亮区均值 143 → **190/255**、过曝像素占比由 1.3% 升至约 4.4%。标定表格见 `painting/p3d-demo.json`（`brightness_pulse_2026-09-13`）。**测量口径**：必须在原生分辨率上量 —— 400×250 降采样会把稀疏点精灵抹掉，读数随粒子微动剧烈跳动（同一配置重复测得 8.6%/12.5% 亮点），据此做出的调参结论不可信。2.7 以上增益被加法混合的饱和吃掉（2.2→2.7 只涨 1.8，而过曝占比翻倍），故停在 2.5。
  - ㊷ **交替节奏**：新增 `uPulse`（默认 **0.55Hz**）—— 每粒子取坐标散列相位 + 行进波 `sin(2π(t·pulse+ph)+3.2x+2.4y)`，alpha 在 0.42–1.0 之间呼吸，**不是整幅一起亮灭**。实测半周期两帧平均亮度差 **9.6**、变化像素 **10.9%**。参数 `?p3dbright=`（0.2–4）、`?p3dpulse=`（0–4）、主站与独立页同名 `?bright=`/`?pulse=`。
  - ㊸ **顺带修掉两个真缺陷**：① **暂停语义** —— 原 `t = uTime * (1.0 - uPaused)` 会把动画时钟归零、按空格画面**跳回起始相位**，且 JS 侧极性交叠用的仍是不带暂停的 t（暂停了还在翻转）；改为 JS 侧累加时钟、暂停即**冻结在当前相位**。② `painting3d.html` 补 `?size=`（与 `?depth/?bright/?pulse` 同规格）。
  - 验收：`check-3d-bright-pulse.js` **8/8**｜`check-3d-toggle.js` **9/9**｜`check-painting3d.js` **6/6**｜`check-3d-flip.js` **6/6**｜`check-3d-orbit.js` **7/7**（断言口径已随第二十四轮更新：「默认静止」+「显式 `?p3dorbit=` 可开启」，旧版「默认在环绕」已被作者否决）｜`check-3d-relief.js` **5/5**｜`check-3d-pause.js` **5/5**（暂停后 1.8s 两帧逐像素一致）；`npm run lint` **0 error**、`scripts/check-goldens.mjs` 全绿。
  - 证据：`painting/p3d-demo.json`（`brightness_pulse_2026-09-13`）、截图 `nf-baseline-artifacts/3d-bright-on.png`、`3d-pulse-a/b.png`、`3d-sweep-b*.png`、`3d-pause.png`。
- 第二十～二十五轮的改动均已 commit 并推到 `codex/batch0-confirm`（tip 见 `git log`）；工作树里的 `painting/codex-*.json`、`painting/assembly-v3-d-*` 与两份申议是随改动一起待提交的产物。
