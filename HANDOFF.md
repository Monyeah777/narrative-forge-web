...
- 径向重映射 S2b：本条分支 `cursor/radial-s2b-thinning-84d9`（叠在 `cursor/radial-s2a-candidates-84d9` 上）。默认 dart-thinning，像素距离，`s0` 二分锁定。Dallas/Scotland/Met n=50601/50441/50471，中心/边缘 2.42/2.61/2.63，NN 1.46/1.55/1.55。不升级 ②/③。现网 sha 未变。证据 `painting/radial-s2b.json`。
...
3. 只做用户这一次点名的事。现网默认 Pixi，介绍/大厅/社区默认 100k bin。`points.bin` 现有 9B `NFPT` 头。不要改弹簧。不要删 `physics()` / `applySpring`。径向重映射 S2b 已落盘。未听到 `继续` 不得开 S2c，也不得把 50k 写进现网。
